...
public interface OtherLendMoneyDao extends BaseDao<OtherLendMoney, Integer> {
    @Modifying
    @Query(value = "INSERT INTO `other_lend_money` (`car_license_num`, `lend_date`, `amount`, `disable`, `note`, `create_time`, `last_modify_time`) " +
            "VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)", nativeQuery = true)
    int insertOtherLendMoney(String carLicenseNum, String lendDate, BigDecimal amount, int disable, String note, long createTime, long lastModifyTime);
    ...
    @Query(value = "SELECT * FROM other_lend_money WHERE car_license_num = ?1 AND lend_date between ?2 AND ?3 AND disable = 0"
            , nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM other_lend_money WHERE car_license_num = ?1 AND lend_date between ?2 AND ?3 AND disable = 0"
            , nativeQuery = true
            , countQuery = "SELECT COUNT(1) FROM other_lend_money WHERE car_license_num = ?1 AND lend_date between ?2 AND ?3 AND disable = 0")
